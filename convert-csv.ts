...
// JSON 파일 읽기
const purchaseHistory: any[] = JSON.parse(fs.readFileSync(inputFile, 'utf-8'))
...
function parseCurrency(currencyString: string) {
  if (!currencyString || currencyString === '-' || currencyString === '무료') {
    return 0
  }
  // 쉼표와 "원"을 제거하고 숫자로 변환
  return (
...
// 탭으로 구분된 데이터 생성
const rows: string[] = purchaseHistory.reverse().map((item: any): string => {
  const emptyObject: Record<NotionHeader, string> = Object.fromEntries(
    notionHeader.map(field => [field, '']),
  )

  // orderData와 inventories 구조 처리
  const orderData = item.orderData || item
  const inventories = item.inventories || []

  // 판매가, 수수료 합산 및 판매가 최대값 기준 코드 추출
  let maxPrice = -Infinity
  let mainCode = ''
  let totalSalePrice = 0
  let totalFee = 0
  for (const inv of inventories) {
    const salePrice = parseCurrency(inv['판매가'])
    const fee = parseCurrency(inv['수수료'])
  ...
  const 구매가 = safeValue(parseCurrency(orderData['총 구매가']))
  const 구매수수료 = safeValue(parseCurrency(orderData['총 수수료']))
  const 포인트 =
    orderData['총 포인트사용'] !== '-' &&
    orderData['총 포인트사용'] !== undefined
      ? safeValue(parseCurrency(orderData['총 포인트사용']))
      : 0

  emptyObject['날짜'] = formatDateString(orderData['거래 일시'])
  emptyObject['제목'] = mainCode
  emptyObject['정가'] = 구매가.toString()
  emptyObject['구매 수수료'] = 구매수수료.toString()
  emptyObject['이벤트 할인'] = '0'
  emptyObject['최종 구매가'] = (구매가 - 구매수수료).toString()
  emptyObject['판매가'] = totalSalePrice.toString()
  emptyObject['판매 수수료'] = totalFee.toString()
  emptyObject['포인트사용(크림)'] = 포인트.toString()
  emptyObject['포인트사용(네이버)'] = '0'
  emptyObject['포인트사용(토스)'] = '0'
  emptyObject['최종 판매가'] = (totalSalePrice - totalFee).toString()
  emptyObject['기타 지출'] = '0'
  emptyObject['기타 수입'] = '0'

  let 결제수단 = orderData['결제수단'] || ''
  emptyObject['메모'] = 결제수단

...
function parseCurrency(currencyString: string) {
  ...
  // 쉼표와 "원"을 제거하고 숫자로 변환
  return parseInt(currencyString.replace(/- /g, '').replace(/,/g, '').replace('원', ''), 10) || 0;
}
...
const rows = purchaseHistory.reverse().map(item => {
  ...
  const 포인트 = item['총 포인트 사용'] !== '-' ? safeValue(parseCurrency(item['총 포인트 사용'])) : 0
...
rows.forEach((row, index) => {
  ...
  if (columns !== headerColumns) {
    console.error(`오류: ${index + 1}번째 행의 열 개수(${columns})가 헤더 열 개수(${headerColumns})와 일치하지 않습니다.`);
    hasError = true;
  }
});
